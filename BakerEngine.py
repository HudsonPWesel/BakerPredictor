...
def fill_char_arr(baker_list_copy: list):
    char_arr.clear()
    rank = 1
...
def elim_char(baker_list_copy, current_rank):
    global char_arr
    chosen_num = np.random.randint(0, len(char_arr))
    chosen_char = char_arr[chosen_num]

    char_arr = [current_char for current_char in char_arr if current_char != char_arr[chosen_num]]
    assign_rank(chosen_char, current_rank)
...
def find_range(baker_list: list) -> int:
    total = 0
    for baker in baker_list:
        total += baker.weight
    return total - 1
...
def simulate_round(baker_list: list):
    currentWeek = 0

    baker_list_copy = baker_list.copy()
    for challenge in range (3):
        fill_char_arr(baker_list_copy)


        for i in range (1, 13):

            elim_char(baker_list_copy, i)
    # print(baker_list[1].cumulative_ranks)
...
def run(epochs: int):
    ...
    for i in range(12):
        baker_list.append(Baker(BAKER_RANKS[i], ("Baker " + str(i + 1), ), CHARS[i]))
    for i in range(1, epochs+1):
        # % Program Finished
        print(str(i) + " | " + "{:.2f}".format((i/epochs)*100) + "%")
        simulate_round(baker_list)
